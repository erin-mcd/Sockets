	...
	while(1){
		...
		//'27' is the escape key.
			...
			//if a file is given to get the dictionary from
				...
				if(inDictionary == 1){
					//this is the word the client typed in:
					//	printf("%ld\n", strlen(recvBuffer));
					send(clientSocket, recvBuffer, bytesReturned, 0);
					//OK
					send(clientSocket, msgOK, strlen(msgOK), 0);
				}
				else{
					//	printf("%ld\n", strlen(recvBuffer));
					//this is the word the client typed in:
					send(clientSocket, recvBuffer, bytesReturned, 0);
					//MISSPELLED
					send(clientSocket, msgMISSPELLED, strlen(msgMISSPELLED), 0);
				}
